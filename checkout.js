...
function updateOrderSummary() {
    ...
    // 更新訂單項目
    if (cart.length === 0) {
        orderItems.innerHTML = '<p style="text-align: center; color: #999; padding: 20px;">購物車是空的</p>';
    } else {
...
function selectDelivery(deliveryType) {
    ...
    document.querySelector(`[value="${deliveryType}"]`).closest('.delivery-option').classList.add('selected');
...
function applyCoupon() {
    ...
    // 顯示已使用的優惠券
    document.getElementById('couponText').textContent = `${coupon.name} - NT$ ${discountAmount}`;
